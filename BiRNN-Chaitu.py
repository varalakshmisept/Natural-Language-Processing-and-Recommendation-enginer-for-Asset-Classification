...
'''
Program Description: This program builds a Bi-LSTM model over the GloVe embeddings on the words obtained after performing the pre-processing over the concatenated string - Order Title and Line Description
'''
...
'''
Description: This function does the pre-processing and the training associated with the Bi-LSTM model over the train and test sets.
'''
...
def bilstm(X_train, X_test, Y_train, Y_test,wordembeddings):
    ...
    '''
    #Run it the first time when you want pickle dumps of X_val and y_val

    with open(config.X_val_DL, 'wb') as f:
        pickle.dump(X_val, f)

    with open(config.y_val_DL, 'wb') as f:
        pickle.dump(y_val, f)
    ...
    model = (Bidirectional(SimpleRNN(64)))(model)
    ...
    model.save(config.birnn_prepocessed_dataset1_chai)
    ...
    plt.savefig('/home/ubuntu/asset_classification/results/birnn_model_dataset1_preprocessed_chai.png')
    
    tf.keras.utils.plot_model(model, to_file=config.birnn_architecture, show_shapes=True) # !!!!!!! CHANGE THIS FOR OTHER MODELS
