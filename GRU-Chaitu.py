...
'''
Program Description: This program builds a GRU model over the GloVe embeddings on the words obtained after performing the pre-processing over the concatenated string - Order Title and Line Description
'''
...
'''
Description: This function does the pre-processing and the training associated with the GRU model over the train and test sets.
'''
...
def bilstm(X_train, X_test, Y_train, Y_test,wordembeddings):
    ...
    model = (GRU(64))(model) # !!!!!!! CHANGE THIS FOR OTHER MODELS
    ...
    model.save(config.gru_prepocessed_dataset1_chai) # !!!!!!! CHANGE THIS FOR OTHER MODELS
    ...
    plt.savefig('/home/ubuntu/asset_classification/results/gru_model_dataset1_preprocessed_chai.png') # !!!!!!! CHANGE THIS FOR OTHER MODELS
    
    tf.keras.utils.plot_model(model, to_file=config.gru_architecture, show_shapes=True) # !!!!!!! CHANGE THIS FOR OTHER MODELS
