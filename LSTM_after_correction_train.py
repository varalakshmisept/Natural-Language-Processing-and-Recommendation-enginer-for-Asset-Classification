...
'''
This program trains the LSTM model on the dataset that is obtained after the correcting the records through the UI. 
'''
...
'''
Description: This function does the pre-processing and the training associated with the LSTM model over the train and test sets.
'''
...
def bilstm(X_train, X_test, Y_train, Y_test,wordembeddings):
    ...
    hist_adam = model.fit(X_train, y_train, batch_size=1000, epochs=1, verbose=1, validation_data=(X_val, y_val),callbacks=callbacks)     #!!!!!!!!!!!!!!!!!!!!!!!CHANGE BATCH SIZE TO 1000 #change epochs to 200
    model.save(config.lstm_prepocessed_dataset1_chai_retrain)
    ...
    plt.savefig('/home/ubuntu/asset_classification/results/lstm_model_dataset1_preprocessed_chai_retrain.png')
    
    tf.keras.utils.plot_model(model, to_file=config.lstm_architecture_retrain, show_shapes=True)
    
    return(y_pred,y_val_class,y_pred_class,y_val_class_argmax,y_pred_class_argmax,report_dict)
...
def lstm_train_after_correction(input_config_file):

    df = pd.read_csv(input_config_file)
    print(df.tail())

    df = df.replace(np.nan, '', regex = True)
    print(df["SPELL_CORRECTED"].isnull().values.any())
    ...
    y_pred,y_val_class,y_pred_class,y_val_class_argmax,y_pred_class_argmax, report_dict = bilstm(X_train, X_test, Y_train, Y_test, wordembeddings)

    return("succesfully trained and saved the model",report_dict)


if __name__=="__main__":
    lstm_train_after_correction(config.deep_learning_dataset_for_retraining)
