...
def bilstm(X_train, X_test, Y_train, Y_test,wordembeddings):
    ...
    model = (SimpleRNN(64))(model) # !!!!!!! CHANGE THIS FOR OTHER MODELS
    ...
    model.save(config.rnn_prepocessed_dataset1_chai) # !!!!!!! CHANGE THIS FOR OTHER MODELS
    ...
    plt.savefig('/home/ubuntu/asset_classification/results/rnn_model_dataset1_preprocessed_chai.png') # !!!!!!! CHANGE THIS FOR OTHER MODELS
    
    tf.keras.utils.plot_model(model, to_file=config.rnn_architecture, show_shapes=True) # !!!!!!! CHANGE THIS FOR OTHER MODELS
